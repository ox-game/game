                    ...
                    if (trustee.IsNotNull() && !trustee.WatchOnly && this.Operater.Wallet is OpenWallet openWallet)
                    {
                        using (ScriptBuilder sb = new ScriptBuilder())
                        {
                            ...
                            if (account != null)
                            {
                                ...
                                foreach (var r in openWallet.GetAssetTrustUTXO().Where(m => m.Value.SpentIndex ==0 && m.Value.OutPut.AssetId.Equals(Room.Request.AssetId) && m.Value.OutPut.ScriptHash.Equals(from.TrustAddress)))
                                {
