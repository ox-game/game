...
{
    public partial class LuckEatSmallMoreBettings : DarkDialog
    {
        public LuckEatSmallMoreBettings(RoomView roomView, Wallet wallet, byte position, MixRoom roominfo, uint index, string dibsaccount)
        {
            InitializeComponent();
            ...
            if (plugin.IsNotNull())
            {
                ...
                if (Bettings != default)
                {
                    ...
                    {
                        var pb = new LuckEatSmallBettingButton(roomView, bs);
