...
namespace OX.UI.Casino
{
    ...
    public class TeamKillTable : Control
    {
        RoomView RoomView;
        ...
        public TeamKillTable(RoomView roomView, Action<PaintEventArgs> action, Action<byte> morebetaction)
        {
        ...
        public void FillBettings(Betting[] bettings)
        {
            ...
            foreach (var bet in bets.GroupBy(m => m.BetRequest.Mark.IsNull() ? null : m.BetRequest.Mark.Target.PublicKey).OrderByDescending(n => n.Sum(p => p.Amount)))
            {
                var betbtn = new TeamKillMergeButton(this.RoomView, bet.Key, bet.ToArray());
